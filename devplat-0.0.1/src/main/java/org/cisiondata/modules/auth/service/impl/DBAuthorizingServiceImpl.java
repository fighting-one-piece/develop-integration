...
public class DBAuthorizingServiceImpl extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        String passwordCipherText = EndecryptUtils.encryptPassword(username, password);
        LOG.info("AuthenticationInfo ------> username:{} password: {}", username, password);
        User user = authService.readUserByAccountAndPassword(username, passwordCipherText);
        if (null == user) {;
        	throw new AuthenticationException("用户名密码错误");
        }
        SimpleAuthenticationInfo simpleAuthenticationInfo = new SimpleAuthenticationInfo(
        		user.getAccount(), password.toCharArray(), getName());
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        LOG.info("user id {} account {} roles: {}", user.getId(), user.getAccount(), roles);
