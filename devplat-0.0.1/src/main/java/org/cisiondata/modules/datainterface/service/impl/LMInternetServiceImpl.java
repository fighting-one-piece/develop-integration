...
public class LMInternetServiceImpl implements ILMInternetService{
	...
	public Map<String, Object> readBankPhone(String phone) throws BusinessException {
		...
		try {
			...
			if(code == 200){
				if(flagcode == 1){
					Map<String,Map<String,Object>> maps = (Map<String,Map<String,Object>>)mapresult.get("data").get("result");
...
//		map.put("index", "S0659,S0477,S0474,S0660,S0661,S0464,S0467,S0462,S0465,S0466,S0468,S0469,S0517,S0677,S0618,S0622,S0623,S0624,S0625,S0670,S0671,S0210,S0211,S0213,S0214,S0049,S0050,S0043,S0044,S0132,S0133,S0657,S0658,S0674,S0493,S0662,S0491,S0470,S0663,S0516,S0471,S0472,S0664,S0652,S0473,S0665,S0653,S0654,S0655,S0492,S0494,S0475,S0476,S0480,S0481,S0503,S0504,S0483,S0656,S0518,S0519,S0520,S0521,S0522,S0523,S0524,S0525,S0526,S0527,S0528,S0529,S0530,S0531,S0487,S0488,S0489,S0490,S0605,S0606,S0607,S0608,S0609,S0610,S0611,S0612,S0613,S0614,S0532,S0533,S0499,S0500,S0145,S0146,S0147,S0148,S0149,S0150,S0495");
		map.put("index", "S0660,S0661,S0464,S0462,S0467,S0465,S0468,S0466,S0469,S0549,S0517,S0574,S0576,S0575,S0279,S0583,S0618,S0622,S0082,S0148");
		String kk = "";
		try {
...
//			String kk = HttpUtils.sendPost(Constants.NORMAL_URL + "/queryQuota", map, Constants.APP_KEY, Constants.APP_KEY_VALUE);
		...
		int sss=(int)(Double.parseDouble(gg.get("errorCode").toString()));
			if(sss==200){
				 ...
				 if (getQuota.containsKey("S0660")) {result.put("银行卡发卡地",getQuota.get("S0660"));}  
				 if (getQuota.containsKey("S0661")) {result.put("是否银联高端客户",getQuota.get("S0661"));}  
				 if (getQuota.containsKey("S0464")) {result.put("卡性质",getQuota.get("S0464"));}  
				 if (getQuota.containsKey("S0467")) {result.put("卡等级",getQuota.get("S0467"));} 
				 if (getQuota.containsKey("S0462")) {result.put("卡种",getQuota.get("S0462"));}  
				 if (getQuota.containsKey("S0465")) {result.put("卡品牌",getQuota.get("S0465"));}  
				 if (getQuota.containsKey("S0466")) {result.put("卡产品",getQuota.get("S0466"));}  
				 if (getQuota.containsKey("S0468")) {result.put("卡名称",getQuota.get("S0468"));}  
				 if (getQuota.containsKey("S0469")) {result.put("发卡行",getQuota.get("S0469"));}  
				 if (getQuota.containsKey("S0517")) {result.put("账户性质",getQuota.get("S0517"));}  
				 if (getQuota.containsKey("S0549")) {result.put("近3月交易总金额（消费类）",getQuota.get("S0549"));}  
				 if (getQuota.containsKey("S0574")) {result.put("近3月交易金额城市分布",getQuota.get("S0574"));}  
				 if (getQuota.containsKey("S0575")) {result.put("近3月交易笔数城市分布",getQuota.get("S0575"));}  
				 if (getQuota.containsKey("S0576")) {result.put("近3月交易天数城市分布",getQuota.get("S0576"));}  
				 if (getQuota.containsKey("S0583")) {result.put("近3月TOP3交易地列表",getQuota.get("S0583"));}  
				 if (getQuota.containsKey("S0279")) {result.put("近1月最大单日累计取现金额 ",getQuota.get("S0279"));} 
				 if (getQuota.containsKey("S0618")) {result.put("是否存在盗卡风险 ",getQuota.get("S0618"));} 
				 if (getQuota.containsKey("S0622")) {result.put("是否发生吞卡",getQuota.get("S0622"));}  
				 if (getQuota.containsKey("S0148")) {result.put("近1月消费高频商户分布",getQuota.get("S0148"));}  
				 if (getQuota.containsKey("S0082")) {result.put("近1月消费城市金额分布",getQuota.get("S0082"));}  
				 String statCode = ss.get("data").get("statCode");
				 if( ("2000").equals(statCode) || ("2001").equals(statCode) || ("8000").equals(statCode) || ("8001").equals(statCode) || ("8002").equals(statCode)) {
