...
$(function(){
	$.ajax({
		type:"get",
		url:projectName+"/admin/aResource/adminMenu",
		data:{"parentId":""},
		dataType:"json",
			...
			if(result.code == 1 && result){
				$.each(result.data,function(index,resource){
					$.ajax({
						...
						type:"get",
						url:projectName+"/admin/aResource/adminMenu",
						data:{"parentId":resource.id},
						dataType:"json",
						success:function(results){
							var str = "<dl><dt><i class='"+resource.icon+"'></i> "+resource.name+"<b></b></dt><dd><ul>";
							if(result.code == 1 && result){
								$.each(results.data,function(indexs,resources){
									str += "<li><a _href='"+projectName+resources.url+"' href='javascript:;'>"+resources.name+"</a></li>"
								})
								str += "</ul></dd></dl>";
								$("#adminMenuDiv").append(str);
							}else {
	...
	$(".menu_dropdown").on("click"," dd li a",function(){
		if($(window).width()<768){
			$(".Hui-aside").slideToggle();
		}
	});
	...
	$(".Hui-aside .menu_dropdown").on("click","a",function(){
		var bStop=false;
		var bStopIndex=0;
