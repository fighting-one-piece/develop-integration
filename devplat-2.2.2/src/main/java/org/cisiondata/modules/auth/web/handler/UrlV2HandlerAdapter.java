...
public class UrlV2HandlerAdapter implements HandlerAdapter, InitializingBean {

	private Logger LOG = LoggerFactory.getLogger(UrlV2HandlerAdapter.class);

	private ObjectMapper objectMapper = new ObjectMapper();

	private TpInterfaceHandlerAdapter tpInterfaceHandlerAdapter = null;
	...
	public void afterPropertiesSet() {
		tpInterfaceHandlerAdapter = SpringBeanFactory.getBean(TpInterfaceHandlerAdapter.class);
		Map<String, IRequestService> beans = SpringBeanFactory.getBeansOfType(IRequestService.class);
	...
	public ModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		try {
			...
			Object result = null;
			boolean isThirdpartInterface = tpInterfaceHandlerAdapter.isThirdpartInterface(path);
			if (isThirdpartInterface) {
				result = tpInterfaceHandlerAdapter.handle(request);
			} else {
			...
			writeResponse(response, result, isThirdpartInterface);
		} catch (Exception e) {
			LOG.error(e.getMessage(), e);
			writeResponse(response, wrapperFailureWebResult(ResultCode.FAILURE, e.getMessage()), false);
		}
		return null;
	}
	...
	private void writeResponse(HttpServletResponse response, Object result, boolean isThirdpartInterface)
