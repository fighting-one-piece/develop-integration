...
public class AccessUserInterfaceController {
	...
	//用户账号数据
	 ...
	 //增加用户账号
		@ResponseBody
		@RequestMapping("/AccessUserInterface/{account}/{interfaceid}/{chargeFla}")
		public WebResult  addUserAccount(@PathVariable String account,@PathVariable int interfaceid,@PathVariable int chargeFla){
			WebResult result=new WebResult();
			try {
				result.setData(accessUserInterfaceService.addUserAccount(account, interfaceid, chargeFla));
				result.setCode(ResultCode.SUCCESS.getCode());
			} catch (Exception e) {
				result.setCode(ResultCode.FAILURE.getCode());
				result.setFailure(e.getMessage());
