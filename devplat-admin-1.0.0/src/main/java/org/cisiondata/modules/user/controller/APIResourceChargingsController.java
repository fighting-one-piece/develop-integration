...
@Controller
@RequestMapping(value = "/apiresourcedffee")
public class APIResourceChargingsController {
	...
	public WebResult QueryUserResourceFields(Long userId,Long resourceId) {
		...
		try {
			result.setData(userResourceChargingService.findChargingsByUserIdAndResourceId(userId, resourceId,5));
	...
	public WebResult UpdateUserResourceFields(Long userId,Long resourceId,String chargings) {
		WebResult result = new WebResult();
		try {
			userResourceChargingService.updateChargingsByUserIdAndResourceId(userId, resourceId, chargings,5);
			result.setData("修改成功");
			result.setResultCode(ResultCode.SUCCESS);
		} catch (BusinessException bu) {
			result.setCode(bu.getCode());
