...
@Controller
@RequestMapping(value = "/resourcedfshow")
public class UserResourceFieldsController {
	...
	public WebResult QueryUserResourceFields(Long userId,Long resourceId) {
		...
		try {
			result.setData(userResourceService.findFieldsByUserIdAndResourceId(userId, resourceId,3));
	...
	public WebResult UpdateUserResourceFields(Long userId,Long resourceId,String fields) {
		WebResult result = new WebResult();
		try {
			userResourceService.updateFieldsByUserIdAndResourceId(userId, resourceId, fields,3);
			result.setData("修改成功");
			result.setResultCode(ResultCode.SUCCESS);
		} catch (BusinessException bu) {
			result.setCode(bu.getCode());
