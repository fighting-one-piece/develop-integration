...
public class Jedis extends BinaryJedis implements JedisCommands, MultiKeyCommands, AdvancedJedisCommands,
	...
	public Set<String> keysCustom(final String pattern) {
		checkIsInMultiOrPipeline();
		client.keys(pattern);
		Set<String> keySet = new HashSet<String>();
		List<byte[]> keyList = client.getBinaryMultiBulkReply();
		if (null == keyList || keyList.size() == 0) return keySet;
		for (byte[] key : keyList) {
			try {
				keySet.add((String) SerializerUtils.read(key));
			} catch (Exception e) {
